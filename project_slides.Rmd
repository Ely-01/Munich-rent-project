...
ggplot(rent99, aes(x = area, y = rent)) +
  ...
  theme_minimal() +
  labs(title = "Rent vs. Living Area",
       x = "Area (sqm)", 
       y = "Rent") +
...
-   Primary Driver: `area` is the strongest predictor of `rent` (0.58), confirming size drives relative price

## 

-   Secondary Factors: `yearc` (0.17) and `location` (0.13) show weak linear links to `rent`, suggesting non-linear effects
...
We can see a slight pattern in the residuals for both area and yearc, indicating that the linear assumption may not be sufficient. We will therefore try to model these variables with smoothing functions in the next step.
...
model2$sp
```
Looking at the contribution of the smoothing functions, we can see that both area and yearc are highly significant ($p < 2e-16$), confirming that non-linear relationships are important for this model.The effective degrees of freedom (edf) for area is 7.63, which is substantially higher than 1. This indicates a highly non-linear and complex relationship between area and rent. 

Similarly, yearc has an edf of 4.97, also confirming non-linearity, though slightly less complex than area. The low smoothing parameter for area ($\approx 0.06$) is consistent with this high edf, as it imposes very little penalty on the curve's complexity.
...
The residual plots show no clear patterns for either area or yearc, indicating that the smoothing functions have effectively captured the non-linear relationships. The residuals are randomly scattered around zero, suggesting that the model is well-specified for these predictors.
...
Comparing Model 2 (with bath) against Model 3 (without), Model 2 performs better on every meaningful metric:
- The variable is highly significant ($p = 2.07 \times 10^{-6}$), proving that bathrooms strongly influence rent prices.
- Better Fit (AIC): Model 2 has a lower AIC (39,080 vs. 39,101). A drop of ~20 points is strong evidence that the model is better.
- Lower Error (GCV): The prediction error score (GCV) is lower in Model 2 (18,801) compared to Model 3 (18,927).
- Explained Variance: The Adjusted $R^2$ is higher in Model 2 (0.512), meaning it explains more of the data's variability.
**Conclusion**: Removing bath makes the model significantly worse. It should be retained.


## Gaussian Model with Log Link

Previously, we observed that rent is right-skewed. To avoid the interpretability issues associated with log-transforming the response variable, we will instead model rent directly. By specifying a log link function within the Gaussian family, we can handle the skewness and enforce positive fitted values while preserving the original scale of the data.
...
While the normality checks appear reasonable, the Residuals vs. Linear Predictor plot reveals a critical violation of the Gaussian assumptions.
...
This indicates that the variance is not constant but rather increases with the mean (heteroscedasticity).


## Gamma Model

Although the Gaussian model with a log link provides a reasonable fit, the Gamma family is theoretically superior for modeling rent. Unlike the Gaussian distribution, which assumes constant variance and allows for negative values, the Gamma distribution is naturally defined for positive, right-skewed data and accounts for heteroscedasticity: this aligns better with economic reality, as higher rents typically exhibit greater variability.
...
The Gamma model achieves the lowest AIC (38,501) and BIC (38,614), indicating the best fit among all considered models. Although its deviance explained (51.2%) is slightly lower than the Gaussian log-link model (53.1%), the substantial drop in information criteria confirms that the Gamma distribution is statistically more appropriate for modeling rent data, effectively capturing its positive skewness and heteroscedasticity.

$\implies$ **`model_gamma` offers the best trade-off between complexity and fit while respecting the physical and economic nature of the data.**
